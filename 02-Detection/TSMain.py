...
modelPath = '03-Classification/Models'
# Load the trained model
model = keras.models.load_model('03-Classification/Models/TSModel5')
# Helper function to extract redness from an image
def returnRedness(img):
	yuv = cv2.cvtColor(img,cv2.COLOR_BGR2YUV)
	y, u, v = cv2.split(yuv)
	return v
# Helper function to threshold an image
def threshold(img,T=150):
	_, img = cv2.threshold(img,T,255,cv2.THRESH_BINARY)
	return img 
# Helper function to find contours in an image
def findContour(img):
